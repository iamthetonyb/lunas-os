      ...
      try {
        await new Promise((res, rej) => {
          const req = http.get(url, (res2) => {
            if (res2.statusCode >= 200 && res2.statusCode < 500) res(); 
            else rej(new Error(`HTTP ${res2.statusCode}`));
          });
          req.on('error', rej);
...
(async () => {
  ...
  try {
    ...
    console.error('[e2e-devtools] Fatal error:', error?.message || error);
    
    // Cleanup on error
    if (devServer) devServer.kill('SIGKILL');
    if (devtools) devtools.kill('SIGKILL');
