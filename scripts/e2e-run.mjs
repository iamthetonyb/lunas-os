      ...
      try {
        await new Promise((res, rej) => {
          const req = http.get(url, (res2) => {
            if (res2.statusCode === 200) res(); else rej(new Error(`HTTP ${res2.statusCode}`));
          });
          req.on('error', rej);
...
function spawnDev() {
  ...
  const child = spawn(DEV_CMD, ['dev'], {
    env: { ...process.env, PORT, BASE_URL, NEXTAUTH_URL: BASE_URL },
...
function runJest() {
  ...
  return new Promise((resolve) => {
    const j = spawn(DEV_CMD, ['jest', '--runInBand'], { stdio: 'inherit', env: { ...process.env, BASE_URL } });
    j.on('exit', (code) => resolve(code ?? 1));
  });
}
...
(async () => {
  ...
  console.error('[e2e-run] fatal:', e?.message || e);
  process.exit(1);
});
